...
The repository does not meet the Thermo-Nuclear Code Quality Review approval bar. The MVP is functional and its current CI passes, but the central Batch workflow remains too implicit and permits an ambiguous submission path that can duplicate paid work.

This plan covers the implementation reviewed at commit `25d8c926ddaa78ab5e9bca5c2b97073efb9605c7`. The remediation branch starts from the current `main`.
